...
from .config import HoughConfig, PeakConfig, PaddleDetOrienterConfig
from .hough import (
    build_hough_angles, hough_accumulator,
    find_hough_peaks, PeakResult,
)
...
class SkewEstimator:
    """
    Hough-based skew estimator that uses an external prior angle
    (from text detection) to filter peaks and select the dominant
    text-line orientation.
    """

    def __init__(self, cfg: PaddleDetOrienterConfig, device: torch.device):
        self.cfg = cfg
        self.device = device
    ...
    def _filter_by_prior(self, peaks: PeakResult, prior_deg: float) -> float:
        ...
        skew_angles = 90.0 - peaks.theta_deg
        votes = peaks.votes
