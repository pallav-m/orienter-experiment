...
def clean_boxes(boxes: List[PolygonBox]) -> List[PolygonBox]:
    """Remove zero-area and fully contained boxes."""
    new_boxes = []
    for box_obj in boxes:
        xs = [p[0] for p in box_obj.polygon]
        ys = [p[1] for p in box_obj.polygon]
        if max(xs) == min(xs) or max(ys) == min(ys):
            continue

        box = box_obj.bbox
        contained = False
        for other_obj in boxes:
            if other_obj.polygon == box_obj.polygon:
                continue
            other = other_obj.bbox
            if box == other:
                continue
            if (box[0] >= other[0] and box[1] >= other[1]
                    and box[2] <= other[2] and box[3] <= other[3]):
                contained = True
                break
        if not contained:
...
def parallel_get_boxes(preds, orig_sizes, include_maps=False):
    ...
    for box in bboxes:
        if box.height < 3 * box.width:
            ...
            box.fit_to_bounds([0, 0, orig_sizes[0], orig_sizes[1]])
